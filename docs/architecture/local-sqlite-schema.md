...
It stores normalized sessions, parser checkpoints, pricing snapshots, and
derived aggregates used by the CLI and desktop surfaces.

## Principles

- Keep raw provider data out of the main database.
- Prefer append-safe imports and deterministic recomputation.
- Keep enough lineage to debug parser issues.
- Optimize for local reads first.
...
### `session_score_factors`

```sql
CREATE TABLE session_score_factors (
...
```sql
CREATE INDEX idx_sessions_provider_started_at
  ON sessions(provider, started_at DESC);

CREATE INDEX idx_sessions_project_path
  ON sessions(project_path);

CREATE INDEX idx_sessions_model
  ON sessions(model);

CREATE INDEX idx_sessions_outcome
  ON sessions(outcome);

CREATE INDEX idx_sessions_task_category
  ON sessions(task_category);

CREATE INDEX idx_sessions_last_activity_at
  ON sessions(last_activity_at DESC);
```

## Storage Policy

- `sessions` is the authoritative normalized table.
- explanation and factor tables make score debugging easy without bloating the
  core row.
- derived daily aggregates may be recomputed if logic changes.
- raw transcript content is intentionally excluded.

## Migration Rule

Schema changes must preserve:

- parser checkpoint continuity
- session id stability
- score reproducibility across versions
