...
import { PAGE_STYLES, MENUBAR_STYLES } from "./styles/index.js";
import {
  escapeHtml,
  formatNumber,
  buildCountdownStr,
  ...
  buildContextAuditHtml,
  getSessionContextHealth,
} from "./rendering/helpers.js";
import { buildMenubarHtml } from "./shell/menubar.js";
import { buildBrandLockup } from "./rendering/brand.js";
import { ActiveSurfaceResolver } from "./shell/active-surface-resolver.js";
import {
  loadPreferences,
  savePreferences,
...
import { applySecurityHeaders } from "./app/security-headers.js";
import { sendError } from "./runtime/error-handler.js";
import {
  getRuntimeStatus,
  setDesktopDatabaseResolution,
  getDesktopDatabaseResolution,
  type DesktopRuntimeStatus,
} from "./runtime/runtime-status.js";
import {
  loadEnvironmentConfig,
  RUNTIME_STARTED_AT,
} from "./runtime/env-config.js";
import {
...
function computeDesktopActiveSurfacePanel(
  ...
  const usagePercent =
    recentWithAudit[0]?.contextAudit?.contextUsagePercent ?? null;
...
function serializeNotificationKey(
  key: WindowContextSignal["key"] | null,
): string | null {
  if (!key) {
    return null;
  }
...
function computeDesktopNotificationPayload(
  ...
  if (event && !decision.gateClosed) {
    notificationCheckpoints.set(
      ...
      updateCheckpoint(nextCheckpoint, {
        ...event,
        providerSessionId:
          resolution?.providerSessionId ??
          recentWithAudit[0]?.providerSessionId ??
          null,
  ...
  const providerLabel =
    resolution?.provider ?? recentWithAudit[0]?.provider ?? "provider";
  const usageLabel =
    panel.usagePercent !== null
      ? `${panel.usagePercent.toFixed(1)}%`
      : "unknown";
  const shouldNotify =
    Boolean(event) &&
    !decision.gateClosed &&
    decision.delivery === "desktop_notification";
...
function buildStepChartSvg(
  ...
  const stepWidth =
    values.length > 1 ? innerWidth / (values.length - 1) : innerWidth;
  ...
  return `<div class="signal-window-chips">
    ${periodIds
      .map((periodId) => {
        ...
        const activeClass =
          activePeriod === periodId ? " signal-window-chip-active" : "";
...
function buildTopologyGrid(sessions: StoredSessionListItem[]): string {
  const cells = Array.from({ length: 35 }, (_, index) => {
    ...
    const toolHeavy =
      (session.analysisConfidence ?? 0) < 0.45 ? " topology-cell-hatched" : "";
      ...
      const sessionSignal =
        session.outcome === "success"
          ...
          : session.outcome === "unknown"
            ...
            : 18;
      const value =
        session.pricingSnapshotId === null
          ? "UNVERIFIED"
...
function buildAssetVolatilityRows(analytics: ReadAnalyticsSnapshot): string {
  ...
  return models
    .map((model, index) => {
      const delta =
        model.averageEfficiency !== null ? model.averageEfficiency - 50 : 0;
      const deltaLabel = `${delta >= 0 ? "+" : ""}${delta.toFixed(1)}%`;
      const deltaClass =
        delta >= 0 ? "asset-delta-positive" : "asset-delta-negative";
      const bars = Array.from({ length: 5 }, (_, barIndex) => {
        const ratio =
          ((model.totalTokens / maxTokens) * 100 + barIndex * 9 - index * 6) %
          100;
...
function buildOverviewHero(
  ...
  const successScores = snapshot.providerSummaries.filter(
    (summary) => summary.averageSuccessScore !== null,
  );
  const avgSuccess =
    successScores.length > 0
      ? successScores.reduce(
          (sum, summary) =>
            sum + (summary.averageSuccessScore ?? 0) * summary.sessions,
          0,
        ) / Math.max(snapshot.sessionCount, 1)
      : null;
  ...
  const riskCount =
    (contextHealth?.nearLimitCount ?? 0) + (contextHealth?.toolHeavyCount ?? 0);
  const latencyMs =
    recentSessions.length > 0
      ? Math.max(
          ...
          Math.round(
            recentSessions.reduce(
              (sum, session) =>
                sum + (Math.log10(session.tokenTotal + 1) * 8 + 6),
              0,
      ...
      : 18;
  const syncLag =
    activeSurfacePanel?.usagePercent != null
...
function buildOverviewKpiDeck(
  ...
  const providersWithSuccess = snapshot.providerSummaries.filter(
    (p) => p.averageSuccessScore !== null,
  );
  const avgSuccessScore =
    providersWithSuccess.length > 0
      ? Math.round(
          providersWithSuccess.reduce(
            (sum, p) => sum + (p.averageSuccessScore ?? 0),
            0,
          ) / providersWithSuccess.length,
        )
      : null;

  const topProvider = snapshot.providerSummaries
    .slice()
    .sort((a, b) => b.totalTokens - a.totalTokens)[0];

  const recentTokens = recentSessions
    .slice(0, 7)
    .reverse()
    .map((session) => session.tokenTotal);
  ...
  const cards = [
    ...
    },
    {
      ...
      value:
        avgSuccessScore !== null
          ? `${avgSuccessScore}%`
          : formatNumber(avgTokensPerSession),
  ...
  return `<section class="kinetic-kpi-grid">${cards
    .map(
      (card) => `
...
function buildOverviewTrendActivity(
  snapshot: ReadSummarySnapshot,
  ...
  const trendValues = sessions
    .slice(0, 12)
    .reverse()
    .map((session) => session.tokenTotal || 1);
  const primaryProvider = snapshot.providerSummaries
    .slice()
    .sort((left, right) => right.totalTokens - left.totalTokens)[0];
  const secondaryProvider = snapshot.providerSummaries
    .slice()
    .sort((left, right) => right.sessions - left.sessions)[1];
...
function buildOverviewProviderIntegrity(
  snapshot: ReadSummarySnapshot,
  activeSurfacePanel: DesktopActiveSurfacePanelData | null,
  activePeriod: string = "1m",
): string {
  const providers = snapshot.providerSummaries
    ...
    .sort(
      (left, right) =>
        (right.averageEfficiency ?? 0) - (left.averageEfficiency ?? 0),
    )
    .slice(0, 4);
    ...
    <div class="provider-integrity-list">
      ${providers
        .map((summary) => {
          ...
          const outputWidth = clampNumber(
            summary.averageSuccessScore ?? 34,
            12,
            100,
          );
          const costWidth = clampNumber(
            summary.averageEfficiency ?? 22,
            8,
            100,
          );
...
function buildOverviewCadenceSection(
  sessions: StoredSessionListItem[],
): string {
      ...
      ${sessions
        ...
        .map((session) => {
          ...
          const cost =
            session.pricingSnapshotId === null
              ? "unknown"
              : formatUsdCompact(session.costTotalUsd);
          const title =
            session.title && session.title.length > 52
...
function buildAnalyticsHero(
  ...
  const bestValue =
    providers
      ...
      .sort(
        (left, right) =>
          (right.averageValueDensityScore ?? 0) -
          (left.averageValueDensityScore ?? 0),
      )[0] ?? null;
  const liveIndex =
    bestValue?.averageValueDensityScore !== null
      ? Math.min(99.9, bestValue.averageValueDensityScore + 32)
      : 98.42;
  const loadValue =
    totalCost > 0
      ? Math.min(99, (totalCost / Math.max(totalTokens / 1000, 1)) * 1000)
      : 42.8;
  const errorLatency =
    analytics.dailyBuckets.length > 0
      ? Math.max(
          8,
          Math.round((totalCost / Math.max(analytics.sessionCount, 1)) * 18),
        )
      : 14;
  ...
  return `<section class="analytics-hero-grid">
    <div class="analytics-hero-main">
      ...
      ${buildMiniBars(
        analytics.dailyBuckets.slice(-8).map((bucket) => bucket.sessions || 1),
        "var(--accent)",
      )}
    </div>
...
function buildAnalyticsValueMatrix(analytics: ReadAnalyticsSnapshot): string {
  ...
  const maxCostPerToken = Math.max(
    ...providers.map((summary) =>
      summary.totalTokens > 0 ? summary.totalCostUsd / summary.totalTokens : 0,
    ),
    0.00001,
  );
  const bubbles = providers
    .map((summary) => {
      const xPct = clampNumber(
      ...
      const size = clampNumber(
        (summary.sessions / Math.max(analytics.sessionCount, 1)) * 140 + 18,
        18,
        72,
      );
      const accent = getProviderAccent(summary.provider);
      ...
      <div class="provider-integrity-list provider-integrity-list-large">
        ${providers
          ...
          .map((summary) => {
            ...
            const outputWidth = clampNumber(
              summary.averageSuccessScore ?? 20,
              12,
              100,
            );
            const costWidth = clampNumber(
              summary.averageEfficiency ?? value,
              10,
              100,
            );
...
function buildAnalyticsComposition(analytics: ReadAnalyticsSnapshot): string {
  ...
  for (const session of sessions) {
    ...
    else if (session.outcome === "mixed" || session.outcome === "partial")
      counts.mixed++;
    else if (session.outcome === "waste" || session.outcome === "failed")
      counts.waste++;
    else counts.unknown++;
  }
        ...
        ${analytics.modelSummaries
          ...
          .map((model) => {
            const width = clampNumber(
              (model.totalTokens /
                Math.max(
                  ...analytics.modelSummaries.map((entry) => entry.totalTokens),
                  1,
                )) *
                100,
              8,
...
function buildActiveSurfaceTruthSection(
  panel: DesktopActiveSurfacePanelData,
): string {
  const resolution = panel.resolution;
  const tier = resolution?.resolutionTier ?? "tier_0_none";
  ...
  const textColor = isFallback
    ...
    : isStrongTruth(tier)
      ...
      : "var(--text-primary)";
  const usageText =
    panel.usagePercent !== null ? `${panel.usagePercent.toFixed(1)}%` : "n/a";
  const providerText = resolution?.provider
    ? escapeHtml(resolution.provider)
    : "unresolved";
  const capabilityRows = [
    ...
    .map(
      ([label, value]) =>
        `<div class="context-limit-row"><span class="context-limit-label">${label}</span><span class="detail-value">${value}</span></div>`,
    )
    .join("");
...
function buildOverviewHtml(
  ...
  modelOptions: { model: string; sessionCount: number }[],
  contextHealth: {
    nearLimitCount: number;
    toolHeavyCount: number;
    topSessions: {
  ...
  const totalTokens = snapshot.providerSummaries.reduce(
    (s: number, p: SessionSummary) => s + p.totalTokens,
    0,
  );
  const totalCost = snapshot.providerSummaries.reduce(
    (s: number, p: SessionSummary) => s + p.totalCostUsd,
    0,
  );
  const providerRows =
    snapshot.providerSummaries.length > 0
      ? snapshot.providerSummaries
          .map((summary: SessionSummary) => buildProviderRow(summary))
          .join("\n")
  ...
  const sessionRows =
    sessions.length > 0
      ? sessions
          .map((session: StoredSessionListItem) => buildSessionRow(session))
          .join("\n")
...
function buildOverviewSuccessCard(snapshot: ReadSummarySnapshot): string {
  ...
  const totalSessions = snapshot.providerSummaries.reduce(
    (s: number, p: SessionSummary) => s + p.sessions,
    0,
  );
  const weightedSuccess = providersWithSuccess.reduce(
  ...
  const avgSuccess =
    totalSessions > 0 ? Math.round(weightedSuccess / totalSessions) : null;
...
function buildVerificationDistributionCard(
  snapshot: ReadSummarySnapshot,
): string {
  return "";
}

function buildOverviewContextHealthSection(contextHealth: {
  nearLimitCount: number;
  toolHeavyCount: number;
  topSessions: {
    id: string;
  ...
  if (
    contextHealth.nearLimitCount === 0 &&
    contextHealth.toolHeavyCount === 0 &&
    contextHealth.topSessions.length === 0
  ) {
    return "";
  }
  ...
  if (contextHealth.nearLimitCount > 0 || contextHealth.toolHeavyCount > 0) {
    const nearLimitColor =
      contextHealth.nearLimitCount > 0 ? "var(--warning)" : "var(--text-muted)";
    const toolHeavyColor =
      contextHealth.toolHeavyCount > 0 ? "var(--warning)" : "var(--text-muted)";
    cards.push(
      `<div class="stat-card" style="border-left:3px solid ${nearLimitColor}"><div class="stat-value" style="color:${nearLimitColor}">${contextHealth.nearLimitCount}</div><div class="stat-label">Near Context Limit</div></div>`,
    );
    cards.push(
      `<div class="stat-card" style="border-left:3px solid ${toolHeavyColor}"><div class="stat-value" style="color:${toolHeavyColor}">${contextHealth.toolHeavyCount}</div><div class="stat-label">Tool-Heavy Sessions</div></div>`,
    );
  }
  ...
  if (contextHealth.topSessions.length > 0) {
    const rows = contextHealth.topSessions
      .slice(0, 5)
      .map((s) => {
        const pct =
          s.contextPercent !== null ? `${s.contextPercent.toFixed(1)}%` : "n/a";
...
function buildAnalyticsSuccessSection(
  analytics: ReadAnalyticsSnapshot,
): string {
  const sections: string[] = [];
  const providersWithSuccess = analytics.providerSummaries.filter(
    (p: SessionSummary) => p.averageSuccessScore !== null,
  );
  ...
  const completedSessions = providersWithSuccess.reduce((sum, p) => {
    ...
    return (
      sum +
      (score >= 70
        ? Math.round(p.sessions * 0.6)
        : Math.round(p.sessions * 0.2))
    );
  }, 0);
  const highConfidenceSessions = providersWithSuccess.reduce((sum, p) => {
    ...
    return (
      sum +
      (conf >= 0.7
        ? Math.round(p.sessions * 0.5)
        : Math.round(p.sessions * 0.15))
    );
  }, 0);
  ...
  const funnelSteps = [
    ...
    {
      label: "Likely completed (score ≥ 70)",
      count: completedSessions,
      color: "var(--success)",
    },
    {
      label: "High confidence (≥ 70%)",
      count: highConfidenceSessions,
      color: "var(--warning)",
    },
  ];
  ...
  sections.push(
    `<h3 style="font-size:13px;color:#6b7280;margin:0 0 8px;font-weight:500">Success Funnel</h3>`,
  );
  ...
  const totalVerified = providersWithSuccess.reduce(
    (sum, p) => sum + (p.verifiedSessions ?? 0),
    0,
  );
  const totalProbable = providersWithSuccess.reduce(
    (sum, p) => sum + (p.probableSessions ?? 0),
    0,
  );
  const totalMissing = providersWithSuccess.reduce(
    (sum, p) => sum + (p.missingVerificationSessions ?? 0),
    0,
  );
  const totalContradicted = providersWithSuccess.reduce(
    (sum, p) => sum + (p.contradictedSessions ?? 0),
    0,
  );
  const totalWithVerif =
    totalVerified + totalProbable + totalMissing + totalContradicted;
  ...
  if (totalWithVerif > 0) {
    const verifSteps = [
      ...
      {
        label: "Contradicted",
        count: totalContradicted,
        color: "var(--critical)",
      },
    ];
    const maxVerif = Math.max(totalWithVerif, 1);
    const verifBars = verifSteps
      .map((step) => {
      ...
      .join("\n");
    sections.push(
      `<h3 style="font-size:13px;color:#6b7280;margin:16px 0 8px;font-weight:500">Verification-State Distribution</h3>`,
    );
    sections.push(verifBars);
  }

  sections.push(
    `<h3 style="font-size:13px;color:#6b7280;margin:16px 0 8px;font-weight:500">Avg Success Score by Provider</h3>`,
  );
  const scoreRows = providersWithSuccess
    .map((p: SessionSummary) => {
      ...
      const conf =
        p.averageAnalysisConfidence !== null
          ? `${(p.averageAnalysisConfidence * 100).toFixed(0)}% conf`
          : "no confidence data";
  ...
  const providersWithRework = providersWithSuccess.filter(
    (p) => p.averageReworkScore !== null,
  );
  if (providersWithRework.length > 0) {
    sections.push(
      `<h3 style="font-size:13px;color:#6b7280;margin:16px 0 8px;font-weight:500">Avg Rework Score by Provider (lower = less churn)</h3>`,
    );
    const reworkRows = providersWithRework
      .map((p: SessionSummary) => {
  ...
  const providersWithValueDensity = providersWithSuccess.filter(
    (p) => p.averageValueDensityScore !== null,
  );
  if (providersWithValueDensity.length > 0) {
    sections.push(
      `<h3 style="font-size:13px;color:#6b7280;margin:16px 0 8px;font-weight:500">Avg Value Density by Provider (higher = more progress per spend)</h3>`,
    );
    const vdRows = providersWithValueDensity
      .map((p: SessionSummary) => {
  ...
  sections.push(
    `<p class="footer-note" style="margin-top:8px">Scores reflect likelihood of useful progress, not just cost efficiency. Confidence indicates evidence quality. Rework and value density are best-effort estimates from available signals.</p>`,
  );

  return sections.join("\n");
}

function buildAnalyticsContextSection(
  analytics: ReadAnalyticsSnapshot,
): string {
  const sessions = analytics.recentSessions ?? [];
  if (sessions.length === 0) {
  ...
  const providerContext: Record<
    string,
    { totalContext: number; sessionCount: number }
  > = {};
  const topByUsage = [...sessions]
    .sort(
      (a, b) =>
        (b.contextAudit?.contextUsagePercent ?? 0) -
        (a.contextAudit?.contextUsagePercent ?? 0),
    )
    .slice(0, 5);
  ...
  for (const session of sessions) {
    ...
    if (ca) {
      ...
      if (!providerContext[prov])
        providerContext[prov] = { totalContext: 0, sessionCount: 0 };
      providerContext[prov].totalContext += session.tokenTotal;
      providerContext[prov].sessionCount++;
    }
  }

  const pressureRows = Object.entries(pressureCounts)
    .map(([state, count]) => {
          ...
          : state === "high"
            ...
            : state === "medium"
              ...
              : state === "low"
                ...
                : "#9ca3af";
      const pct =
        sessions.length > 0 ? Math.round((count / sessions.length) * 100) : 0;
  ...
  sections.push(
    `<h3 style="font-size:13px;color:#6b7280;margin:0 0 8px;font-weight:500">Context Pressure Distribution</h3>`,
  );
  sections.push(pressureRows);

  const providerRows = Object.entries(providerContext)
    .sort((a, b) => b[1].totalContext - a[1].totalContext)
    .slice(0, 5)
    .map(([prov, data]) => {
  ...
  if (providerRows) {
    sections.push(
      `<h3 style="font-size:13px;color:#6b7280;margin:16px 0 8px;font-weight:500">Context by Provider</h3>`,
    );
  ...
  if (topByUsage.length > 0) {
    sections.push(
      `<h3 style="font-size:13px;color:#6b7280;margin:16px 0 8px;font-weight:500">Top Sessions by Context Usage</h3>`,
    );
    const topRows = topByUsage
      .map((s) => {
...
function buildDetailSuccessGrid(session: StoredSessionDetail): string {
  ...
  if (sa.completionState) {
    const cls =
      sa.completionState === "completed"
        ? "badge-ok"
        : sa.completionState === "partial"
          ? "badge-warn"
          : "badge-warn";
    rows.push(
      `<div class="detail-label">Completion</div><div class="detail-value"><span class="badge ${cls}">${escapeHtml(sa.completionState)}</span></div>`,
    );
  }
  if (sa.verificationState) {
    const cls =
      sa.verificationState === "verified"
        ? "badge-ok"
        : sa.verificationState === "probable"
          ? "badge-warn"
          : sa.verificationState === "contradicted"
            ? "badge-warn"
            : "badge-unknown";
    rows.push(
      `<div class="detail-label">Verification</div><div class="detail-value"><span class="badge ${cls}">${escapeHtml(sa.verificationState)}</span></div>`,
    );
  }
    ...
    const color =
      sa.successScore >= 70
        ...
        : sa.successScore >= 40
          ...
          : "var(--critical)";
    rows.push(
      `<div class="detail-label">Success Score</div><div class="detail-value" style="color:${color}">${sa.successScore}</div>`,
    );
  }
  if (sa.executionQualityScore !== null) {
    rows.push(
      `<div class="detail-label">Execution Quality</div><div class="detail-value">${sa.executionQualityScore}</div>`,
    );
  }
  if (sa.reworkScore !== null) {
    rows.push(
      `<div class="detail-label">Rework Score</div><div class="detail-value">${sa.reworkScore}</div>`,
    );
  }
  if (sa.valueDensityScore !== null) {
    rows.push(
      `<div class="detail-label">Value Density</div><div class="detail-value">${sa.valueDensityScore}</div>`,
    );
  }
  if (sa.analysisConfidence !== null) {
    rows.push(
      `<div class="detail-label">Analysis Confidence</div><div class="detail-value">${(sa.analysisConfidence * 100).toFixed(0)}%</div>`,
    );
  }
...
function buildDetailSuccessSection(session: StoredSessionDetail): string {
  ...
  const positiveSignals = sa.successSignals.filter(
    (s: { direction: string }) => s.direction === "positive",
  );
  const negativeSignals = sa.successSignals.filter(
    (s: { direction: string }) => s.direction === "negative",
  );
  ...
  if (positiveSignals.length > 0) {
    html +=
      '<h4 style="color:var(--success-text);margin:8px 0 4px">Positive signals</h4><ul>';
  ...
  if (negativeSignals.length > 0) {
    html +=
      '<h4 style="color:var(--critical-text);margin:8px 0 4px">Negative signals</h4><ul>';
...
function buildResetCell(summary: SessionSummary): string {
  if (
    summary.resetWindowRemainingPercent === null &&
    summary.resetWindowKind === null
  ) {
...
function buildFilterStateHtml(
  activeProvider: string | null,
  activeModel: string | null,
  activeQ: string | null,
  listResult: { total: number } | null,
): string {
  ...
  if (activeProvider)
    pills.push(
      `<span class="filter-pill">provider: ${escapeHtml(activeProvider)}</span>`,
    );
  if (activeModel)
    pills.push(
      `<span class="filter-pill">model: ${escapeHtml(activeModel)}</span>`,
    );
  if (activeQ)
    pills.push(
      `<span class="filter-pill">search: ${escapeHtml(activeQ)}</span>`,
    );
...
function buildPaginationHtml(
  ...
  const nextPage =
    listResult.page < listResult.totalPages ? listResult.page + 1 : null;

  const prevUrl =
    prevPage !== null
      ? `/?page=${prevPage}${params.toString() ? "&" + params.toString() : ""}`
      : null;
  const nextUrl =
    nextPage !== null
      ? `/?page=${nextPage}${params.toString() ? "&" + params.toString() : ""}`
      : null;
...
function buildAnalyticsHtml(
  ...
  const maxProviderCost = Math.max(
    ...analytics.providerSummaries.map((p: SessionSummary) => p.totalCostUsd),
    0.01,
  );
  const providerDistBars = analytics.providerSummaries
    .map((p: SessionSummary) => {
  ...
  const maxModelTokens = Math.max(
    ...analytics.modelSummaries.map((m: ModelSummary) => m.totalTokens),
    1,
  );
  const modelDistBars = analytics.modelSummaries
    .slice(0, 10)
    .map((m: ModelSummary) => {
  ...
  const maxDailyTokens = Math.max(
    ...dailyBuckets.map((d: DailyBucket) => d.totalTokens),
    1,
  );

  const dailyRows = dailyBuckets
    .map((d: DailyBucket) => {
      return `<tr>
  ...
  <div class="section">
    ...
    ${
      session.scoreFactors.length > 0
        ? `<ul class="factor-list">${session.scoreFactors
            .map((f: { direction: string; label: string; impact: number }) => {
              const cls =
...
function handleRequest(
  ...
  const { path, sessionId, provider, model, q, page, mode, period } =
    parseUrlPath(rawUrl);
  ...
  if (path === "/menubar") {
    ...
    try {
      ...
      periodSnapshot = readService.getSummarySnapshotForPeriod(
        activePeriod as "1h" | "1d" | "7d" | "1m" | "all",
      );
      if (periodSnapshot.sessionCount > 0) {
        periodSessions = readService.listRecentSessionsForPeriod(
          activePeriod as "1h" | "1d" | "7d" | "1m" | "all",
          { limit: 5 },
        );
      }
    ...
    let periodCost = 0;
    if (periodSnapshot) {
      periodCost = periodSnapshot.providerSummaries.reduce(
        (sum, p) => sum + p.totalCostUsd,
        0,
      );
    }
    ...
    try {
      const analytics = readService.getAnalyticsSnapshotForPeriod(
        activePeriod as "1h" | "1d" | "7d" | "1m" | "all",
      );
      ...
      if (activeSurfaceState.isAvailable && activeSurfaceState.resolution) {
        ...
        if (activeSurfaceState.resolution.key) {
          ...
          windowContextSignal = {
            ...
            lastCrossedAt: null,
            resolutionConfidence: activeSurfaceState.resolution
        ...
        if (latestSession?.contextAudit) {
          ...
          windowContextSignal = {
            key: {
              provider: latestSession.provider as ProviderId,
              externalWindowId: "latest-session-fallback",
            },
        ...
        const sessionState = createActiveSurfaceStateFromSessions(
          recentWithAudit as any,
        );
        activeSurfaceResolution = sessionState.resolution;
      }
    } catch (caught) {
      process.stderr.write(
        `[WARN] failed to compute menubar active surface: ${caught instanceof Error ? caught.message : String(caught)}\n`,
      );
    ...
    response.end(
      buildMenubarHtml(
        ...
        windowContextSignal,
        activeSurfaceResolution,
        { period: periodLabel, cost: periodCost },
      ),
    );
    return;
  }
  ...
  if (path === "/analytics") {
    ...
    if (error) {
      response.writeHead(500, { "Content-Type": "text/html; charset=utf-8" });
      response.end(
        buildErrorHtml("analytics", error, runtimeStatus ?? undefined),
      );
      return;
    }
    ...
    if (!analytics || analytics.sessionCount === 0) {
      ...
      response.end(
        buildEmptyHtml(
          "analytics",
          runtimeStatus ?? getRuntimeStatus(readService, prefs, 30),
          (runtimeStatus?.totalSessionCount ?? 0) > 0
            ? "no-window-data"
            : "no-imported-data",
          activePeriod,
        ),
    ...
    response.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
    response.end(
      buildAnalyticsHtml(analytics, activePeriod, activeSurfacePanel),
    );
    return;
  }
  ...
  if (path === "/" || path === "/index.html") {
      ...
      try {
        ...
        response.end(
          buildErrorHtml(
            ...
            getRuntimeStatus(readService, prefs),
          ),
        );
      }
      return;
    ...
    try {
      ...
      if (snapshot.sessionCount > 0) {
        listResult = readService.listSessionsWithCountForPeriod(activePeriod, {
    ...
    if (error) {
      response.writeHead(500, { "Content-Type": "text/html; charset=utf-8" });
      response.end(
        buildErrorHtml("overview", error, getRuntimeStatus(readService, prefs)),
      );
      return;
    }
    ...
    if (!snapshot || snapshot.sessionCount === 0) {
      ...
      response.end(
        buildEmptyHtml(
          "overview",
          runtimeStatus,
          runtimeStatus.totalSessionCount > 0
            ? "no-window-data"
            : "no-imported-data",
          activePeriod,
        ),
    ...
    try {
      ...
      for (const s of recentWithAudit) {
        ...
        if (ca) {
          if (
            ca.contextPressureState === "high" ||
            ca.contextPressureState === "critical"
          )
            nearLimitCount++;
          if (ca.toolCallCount > 50) toolHeavyCount++;
          sessionContextMap.push({
            id: s.id,
            title: s.title,
            providerSessionId: s.providerSessionId,
            contextPercent: ca.contextUsagePercent,
          });
        }
      }
      contextHealth =
        sessionContextMap.length > 0
          ? {
              nearLimitCount,
              toolHeavyCount,
              topSessions: sessionContextMap.sort(
                (a, b) => (b.contextPercent ?? 0) - (a.contextPercent ?? 0),
              ),
            }
          : null;
      activeSurfacePanel = computeDesktopActiveSurfacePanel(
      ...
      process.stderr.write(
        `[WARN] failed to compute overview context health: ${caught instanceof Error ? caught.message : String(caught)}\n`,
      );
    }
...
function buildMenubarEmptyHtml(
  runtimeStatus: DesktopRuntimeStatus | null,
): string {
  const copy =
    runtimeStatus && runtimeStatus.totalSessionCount > 0
...
function buildSessionMeter(sessionCount: number): string {
  ...
  const health =
    sessionCount < 50 ? "healthy" : sessionCount < 100 ? "warn" : "critical";
...
function buildMinimalProviderRows(summaries: SessionSummary[]): string {
  return summaries
    .map((p: SessionSummary) => {
      ...
      return (
        '<div class="provider-row"><span class="provider-dot provider-dot-' +
        health +
        '"></span><span class="provider-name">' +
        escapeHtml(p.provider) +
        '</span><span class="provider-cost">' +
        costStr +
        "</span></div>"
      );
    })
...
function main(): void {
  ...
  server.listen(PORT, "127.0.0.1", () => {
    process.stdout.write(
      `Token Tracker desktop running at http://localhost:${PORT}\n`,
    );
    ...
    process.stdout.write(
      `File watcher: active (${prefs.refreshCadenceSeconds}s interval)\n`,
    );
    process.stdout.write(
      `Default analytics window: ${prefs.defaultAnalyticsWindowDays} days\n`,
    );
  });
}
