...
import type { SessionData } from "./leaderboard/scoring.js";
import { LeaderboardDatabase } from "./storage/db.js";
...
function createTempLocalDb(sessionCount: number): {
  db: TtmDatabase;
  path: string;
} {
...
describe("scoring", () => {
  it("rejects users with fewer than 3 sessions (minimum participation threshold)", () => {
    ...
    assert.strictEqual(
      result,
      null,
      "Should return null for users below minimum threshold",
    );
  ...
  it("accepts users with exactly 3 sessions", () => {
    ...
    assert.ok(
      result !== null,
      "Should return a score for users at minimum threshold",
    );
  });
  ...
  it("ranks by efficiency score descending, then session count descending", () => {
    ...
    assert.strictEqual(
      entries[0].userId,
      "3",
      "Highest efficiency should be rank 1",
    );
    assert.strictEqual(entries[1].rank, 2);
    assert.strictEqual(
      entries[1].userId,
      "2",
      "Same efficiency but more sessions should be rank 2",
    );
    assert.strictEqual(entries[2].rank, 3);
    assert.strictEqual(
      entries[2].userId,
      "1",
      "Same efficiency but fewer sessions should be rank 3",
    );
  });
});
...
describe("database period-aware queries", () => {
  ...
  it("getUserSessions(windowDays) returns correctly filtered rows", () => {
    ...
    assert.strictEqual(
      weekSessions.length,
      1,
      "Should return only recent session for 7-day window",
    );
    assert.strictEqual(weekSessions[0].sessionId, "recent");
    assert.strictEqual(
      monthSessions.length,
      1,
      "Should return only recent session for 30-day window",
    );
    assert.strictEqual(
      allSessions.length,
      2,
      "Should return all sessions for all-time window",
    );
  });
  ...
  it("getSnapshot(teamId, windowDays) returns the correct snapshot", () => {
    ...
    assert.strictEqual(
      nullSnapshot,
      null,
      "Should return null for non-existent window",
    );
  });
});
...
describe("real ingestion via syncLocalSessions", () => {
  ...
  beforeEach(() => {
    ...
    leaderboardDb.upsertGitHubUser({
      githubId: 12345,
      username: "testuser",
      displayName: "Test User",
      avatarUrl: null,
  ...
  afterEach(() => {
    ...
    try {
      rmSync(leaderboardDbPath, { recursive: true, force: true });
    } catch {
      /* ignore */
    }
  ...
  it("syncLocalSessions ingests expected fields and excludes sensitive ones", () => {
    const result = syncLocalSessions(
      leaderboardDb,
      12345,
      "test-team",
      30,
    ...
    assert.ok(
      ...
      "Should ingest at least some sessions",
    );
    ...
    for (const field of sensitiveFields) {
      assert.ok(
        !keys.includes(field),
        `Should NOT have sensitive field: ${field}`,
      );
    }
  });
  ...
  it("syncLocalSessions can ingest more than 50 sessions end to end", () => {
    ...
    const syncResult = syncLocalSessions(
      leaderboardDb,
      12345,
      "test-team",
      3650,
    ...
    assert.ok(
      syncResult.syncedSessions >= 100,
      `Should ingest at least 100 sessions (got ${syncResult.syncedSessions})`,
    );
    ...
    assert.ok(
      sessions.length >= 100,
      `Leaderboard DB should have at least 100 sessions (got ${sessions.length})`,
    );

    // Cleanup
    try {
      rmSync(largeLocalDbPath.replace("test.sqlite", ""), {
        recursive: true,
        force: true,
      });
    } catch {
...
describe("real app HTTP endpoints", () => {
  ...
  describe("/api/leaderboard period handling", () => {
    ...
    it("returns the snapshot matching the requested period (week)", async () => {
      const { statusCode, body } = await runAppRequest(
        app,
        "GET",
      ...
      assert.strictEqual(
        data.snapshot?.windowDays,
        7,
        "Snapshot should match requested period",
      );
    });
    ...
    it("returns the snapshot matching the requested period (all_time)", async () => {
      const { statusCode, body } = await runAppRequest(
        app,
        "GET",
        "/api/leaderboard?period=all_time",
      );
      assert.strictEqual(statusCode, 200);
      const data = JSON.parse(body);
  ...
  describe("/leaderboard drawer payload hardening", () => {
    ...
    it("base64-encodes member payloads so special characters round-trip safely", async () => {
      ...
      const decodedPayloads = matches.map(
        (match) =>
          JSON.parse(Buffer.from(match[1], "base64").toString("utf8")) as {
            displayName: string;
            username: string;
          },
      );

      const specialPayload = decodedPayloads.find(
        (payload) => payload.username === `o'brien<&quot;user>`,
      );
      assert.ok(
        specialPayload,
        "Should preserve the special-character username",
      );
      assert.strictEqual(
        specialPayload?.displayName,
        `Display "Quoted" O'Brien <Admin> & &#39;`,
      );
      assert.ok(
        !body.includes("data-member="),
        "Should no longer embed raw JSON in HTML attributes",
      );
    });
  ...
  describe("/api/compute-snapshot admin protection", () => {
    ...
    it("returns 403 when admin key is configured and header is missing", async () => {
      const { statusCode } = await runAppRequest(
        app,
        "POST",
    ...
    it("succeeds when admin key matches", async () => {
      ...
      assert.strictEqual(
        statusCode,
        200,
        "Should return 200 with correct admin key",
      );
      const data = JSON.parse(body);
  ...
  describe("GitHub OAuth and session handling", () => {
    ...
    it("redirects to GitHub OAuth when configured", async () => {
      ...
      assert.ok(
        findSetCookie(
          { "set-cookie": toSetCookieArray(response.headers["Set-Cookie"]) },
          "oauth_state=",
        ),
    ...
    it("completes OAuth callback, persists user, and creates a real session", async () => {
      ...
      const stateCookie = findSetCookie(
        { "set-cookie": toSetCookieArray(startResponse.headers["Set-Cookie"]) },
        "oauth_state=",
      );
      const oauthState = extractCookieValue(stateCookie, "oauth_state");
      assert.ok(oauthState, "OAuth start should set state cookie");

      const callbackResponse = await runAppRequest(
        app,
        "GET",
        `/auth/github/callback?code=test-code&state=${oauthState}`,
        {
    ...
    it("rejects invalid OAuth state and clears nothing else", async () => {
      ...
      const response = await runAppRequest(
        app,
        "GET",
        "/auth/github/callback?code=test-code&state=bad-state",
        {
    ...
    it("disconnect clears the authenticated session safely", async () => {
      ...
      const startResponse = await runAppRequest(app, "GET", "/auth/github");
      const oauthState = extractCookieValue(
        findSetCookie(
          {
            "set-cookie": toSetCookieArray(startResponse.headers["Set-Cookie"]),
          },
      ...
      const sessionValue = extractCookieValue(
        findSetCookie(
          {
            "set-cookie": toSetCookieArray(
              callbackResponse.headers["Set-Cookie"],
      ...
      const disconnectResponse = await runAppRequest(
        app,
        "POST",
        "/auth/disconnect",
        {
      ...
      assert.ok(
        ...
        "Disconnect should clear the session cookie",
      );
